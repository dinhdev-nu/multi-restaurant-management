...
/* ─────────────────────────── config maps ─────────────────────────── */
const STATUS_CONFIG: Record<OrderStatus, StatusConfigItem> = {
...
const PAYMENT_CONFIG: Record<PaymentStatus, PaymentConfigItem> = {
...
const formatTimeUntilExpired = (iso?: string): { text: string; expired: boolean; urgent?: boolean } | null => {
  if (!iso) return null;
...
const OrderCard = ({ order, onConfirmOrder, onReorderDraft }: OrderCardProps) => {
          ...
          <div className="min-w-0">
            ...
            {expiredInfo ? (
              <span className={`ml-2 text-[11px] font-medium ${
                expiredInfo.expired ? 'text-error' :
      ...
      <div className="p-4 border-b border-border flex-shrink-0">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-semibold text-foreground">Xác nhận đơn hàng</h1>
            ...
            return (
              ...
              >
                ...
                <span className={`
                  ...
                  ${active ? 'bg-white/20 text-primary-foreground' : 'bg-background text-muted-foreground'}
                `}>
                  {count}
                </span>
              </button>
