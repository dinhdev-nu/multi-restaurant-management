...
import {
  Shield, Lock, Key, RefreshCw, Globe, AlertTriangle, Download, Trash2
} from "lucide-react";
...
export function SecurityTab() {
  ...
  const handleChangePassword = async () => {
    if (newPassword !== confirmNewPassword) { setPasswordError("Mật khẩu mới không khớp"); return; }
    setPasswordError(null); setPasswordSuccess(false); setIsChangingPassword(true);
    try {
  ...
  const handleToggle2fa = async () => {
    ...
    try {
      if (profile?.two_factor_enabled) { await disable2fa(twoFaPassword); toast.success("Đã tắt xác thực 2 lớp"); }
      else { await enable2fa(twoFaPassword); toast.success("Đã bật xác thực 2 lớp"); }
      setTwoFaPassword(""); setShow2faForm(false); await fetchProfile();
    } catch (error) {
      toast.error(toAppError(error, "Không thể cập nhật 2FA").message);
    } finally { setIsTwoFaLoading(false); }
  };
        ...
        <CardContent className="space-y-6">
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <InputGroup className="max-w-md">
                <InputGroupAddon align="inline-start">
                  <InputGroupText><Lock className="w-4 h-4" /></InputGroupText>
                </InputGroupAddon>
                <InputGroupInput
            ...
            <div className="space-y-2">
              ...
              <InputGroup className="max-w-md">
                <InputGroupAddon align="inline-start">
                  <InputGroupText><Key className="w-4 h-4" /></InputGroupText>
                </InputGroupAddon>
                <InputGroupInput
            ...
            <Button variant="outline" onClick={handleChangePassword}
              ...
              {isChangingPassword ? (<><RefreshCw className="w-4 h-4 mr-2 animate-spin" />Updating...</>) : "Update Password"}
            </Button>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between p-4 rounded-lg bg-secondary/50 border border-border">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button variant="outline" size="sm" onClick={() => { setShow2faForm((v) => !v); setTwoFaPassword(""); }}>
          ...
          {show2faForm && (
            <div className="px-4 py-3 rounded-lg border border-border bg-secondary/20 space-y-3 animate-in fade-in slide-in-from-top-1 duration-200">
              <p className="text-sm text-muted-foreground">
                {profile?.two_factor_enabled ? "Enter your password to disable 2FA." : "Enter your password to enable 2FA."}
              </p>
              <div className="flex items-center gap-2">
                ...
                <Button size="sm" variant={profile?.two_factor_enabled ? "destructive" : "default"}
                  onClick={handleToggle2fa} disabled={isTwoFaLoading || !twoFaPassword.trim()}>
                  {isTwoFaLoading ? <RefreshCw className="w-4 h-4 animate-spin" /> : profile?.two_factor_enabled ? "Disable 2FA" : "Enable 2FA"}
                </Button>
              </div>
            </div>
          )}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium ${isAlert ? "text-destructive" : "text-foreground"}`}>{event.label}</p>
                    <p className="text-xs text-muted-foreground">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="p-4 rounded-lg bg-destructive/5 border border-destructive/20 space-y-3">
            ...
            <div className="space-y-2">
              ...
              <InputGroup>
                <InputGroupInput
                  id="deleteConfirm" value={deleteConfirm}
